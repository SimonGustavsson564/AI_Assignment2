...
env = TransformReward(env, lambda r: 1 if r == 1 else r-0.04)  # -0.04 per step -> prefer the shortest path

observation, info = env.reset(seed=42)
action = env.action_space.sample()
print("Starting")
terminated_i = 0
Q_table = qlearning.QTable(0.99, 0.1, 0.5, env.action_space.n, env.observation_space.n)  # gamma, alpha, epsilon, actions, states
### TRAINING LOOP
...
try:
    # Stop criteria: Q-table barely changes for 1000 episodes in a row
    convergence = 0.00001
    stable_episodes = 0
...
try:
    while True:
        ...
        if terminated or truncated:
            ...
            if episodes >= 10:
                print("Test ended after 10 episodes")
                break
